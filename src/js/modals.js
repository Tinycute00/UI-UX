/* ── MODALS ── */
export function om(id){document.getElementById(id)?.classList.add('open')}
export function cm(id){document.getElementById(id)?.classList.remove('open')}
...
export function toast(msg,type='ts'){
  ...
  w.appendChild(t);
  setTimeout(()=>{t.style.cssText='opacity:0;transform:translateX(10px);transition:.25s';setTimeout(()=>t.remove(),250)},3200);
}
