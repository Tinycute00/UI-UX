...
export function setWorkDetail(
  ...
  var diffStr =
    diff >= 0
      ? '<span style="color:var(--green)">▲ 超前 ' + diff + '%</span>'
      : '<span style="color:var(--red)">▼ 落後 ' + Math.abs(diff) + '%</span>';
  ...
  ae.textContent = actual + '%';
  ae.style.color = col;
  var al = document.getElementById('wd-actual-lbl');
  al.innerHTML = actual + '% ' + diffStr;
  al.style.color = col;
  var ab = document.getElementById('wd-actual-bar');
  ab.style.width = actual + '%';
  ab.style.background = col;
...
export function setIRDetail(id, loc, item, date, result, l1, l2, l3) {
  ...
  const rMap = {
    通過: '<span class="tag tg">通過</span>',
    進行中: '<span class="tag ta">進行中</span>',
    退件: '<span class="tag tr">退件</span>',
  };
  document.getElementById('ird-result').innerHTML = rMap[result] || result;
  const lMap = {
    pass: '<span class="tag tg">合格</span>',
    fail: '<span class="tag tr">不合格</span>',
    pending: '<span class="tag ta">待審</span>',
  };
  ['l1', 'l2', 'l3'].forEach((k, i) => {
    const v = [l1, l2, l3][i];
    document.getElementById('ird-' + k).innerHTML = lMap[v] || '<span class="tag tx">—</span>';
  });
...
export function setNCRDetail(id, title, type, vendor, deadline, status, desc) {
  ...
  var sMap = {
    open: '<span class="tag tr">開立中</span>',
    overdue: '<span class="tag tr">改善逾期</span>',
    improving: '<span class="tag ta">改善中</span>',
    pending_re: '<span class="tag tb">待複驗</span>',
    overdue_re: '<span class="tag tr">複驗逾期</span>',
    closed: '<span class="tag tg">已關閉</span>',
  };
  const isOv = status === 'overdue' || status === 'overdue_re';
  document.getElementById('ncrd-deadline').innerHTML =
    (isOv ? '<span style="color:var(--red)">' : '') +
    '改善期限：' +
    deadline +
    (isOv ? ' ─ 已逾期</span>' : '');
...
export function setSubDetail(name, work, pct, status, contract, period, scope) {
  ...
  var sMap = {
    施工中: '<span class="tag tg">施工中</span>',
    進度落後: '<span class="tag ta">進度落後</span>',
    進場準備: '<span class="tag to">進場準備</span>',
  };
  ...
  var d = {
    ...(contractData[name] || { contract: '—', period: '—', scope: '—' }),
    contract: contract || contractData[name]?.contract || '—',
    period: period || contractData[name]?.period || '—',
    scope: scope || contractData[name]?.scope || '—',
  };
  var sc = document.getElementById('subd-contract');
  if (sc) sc.textContent = d.contract;
  var sp = document.getElementById('subd-period');
  if (sp) sp.textContent = d.period;
  var ss = document.getElementById('subd-scope');
  if (ss) ss.textContent = d.scope;
  subTab(1);
}
...
export function saveWorkLog() {
  ...
  entry.style.cssText =
    'background:var(--s3);border:1px solid var(--bd1);border-radius:var(--r-sm);padding:13px;margin-bottom:10px';
  entry.innerHTML =
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:9px"><span class="fm-font" style="font-size:10px;color:var(--gold-txt)">' +
    date +
    '</span><span class="tag to">' +
    rating.split(' ')[0] +
    '</span></div>' +
    '<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;margin-bottom:8px">' +
    '<div><div class="fm-font" style="font-size:8px;color:var(--tx3);margin-bottom:3px">出工</div><div style="font-size:14px;font-weight:600;color:var(--tx1)">' +
    workers +
    ' 人</div></div>' +
    '<div><div class="fm-font" style="font-size:8px;color:var(--tx3);margin-bottom:3px">進度</div><div style="font-size:14px;font-weight:600;color:var(--green)">' +
    progress +
    '%</div></div>' +
    '<div><div class="fm-font" style="font-size:8px;color:var(--tx3);margin-bottom:3px">品質</div><span class="tag tg">' +
    quality +
    '</span></div>' +
    '<div><div class="fm-font" style="font-size:8px;color:var(--tx3);margin-bottom:3px">配合</div><span class="tag tg">' +
    coop +
    '</span></div></div>' +
    '<div style="font-size:11px;color:var(--tx3)">備註：' +
    notes +
    '</div>';
...
export function setBillingDetail(period, range, pct, applied, approved, paid) {
  ...
  document.getElementById('bd-approved').innerHTML =
    '<strong style="color:var(--gold-txt)">' + approved + '</strong>';
...
export function setMatDetail(date, name, vendor, qty, unit, result, report) {
  ...
  document.getElementById('matd-result').innerHTML =
    '<span class="tag ' + (result === '合格' ? 'tg' : 'tr') + '">' + result + '</span>';
...
export function setMatQC(date, name, vendor, qty, unit, sample) {
  ...
  document.getElementById('matqc-sample').innerHTML =
    sample === '待'
      ? '<span class="tag ta">待送驗</span>'
      : '<span class="tag tg">' + sample + '</span>';
}
